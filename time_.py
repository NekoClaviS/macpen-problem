    ...
    def update(self):
        ...
        for i in range(self.grid.rows):
            for j in range(self.grid.cols):
                ...
                if self.fight:  # fighting on?
                    g, u, t = cell.get_population()
                    if g >= u:
                        to_die = []
                        for j in range(u):
                            cell.pacmen[TRAIT.GRATEFUL][j].kill(
                                cell.pacmen[TRAIT.UNGRATEFUL][j], cell
                            )
                            to_die.append(cell.pacmen[TRAIT.UNGRATEFUL][j])
                        for p in to_die:
                            p.die(cell)
                        del to_die
                    else:
                        to_die = []
                        for j in range(g):
                            cell.pacmen[TRAIT.UNGRATEFUL][j].kill(
                                cell.pacmen[TRAIT.GRATEFUL][j], cell
                            )
                            to_die.append(cell.pacmen[TRAIT.GRATEFUL][j])
                        for p in to_die:
                            p.die(cell)
                        del to_die
                ...
                for trait in cell.pacmen.keys():
                    for pacman in cell.pacmen[trait]:
                        ...
                        if pacman.food < 0:
                            ...
                            idx = i * self.grid.cols + j
                            cell.pacmen[pacman.trait].remove(
                                pacman
